...
for mooring_location = 1:4
    ...
    for yr = loc_yrs{mooring_location}
        ...
        % deleting all the variables except those listed under
        ...
        %% Reading draft data to variables        
        ...
        if estimate_hourly==1
            ...
            clear h_LI2
            for n = 1:length(t_LI)
                [f,xi]=ksdensity(h_reshape(:,n)); % making a karnel PDF from the data
                [fmax imax] = max(f); % finding maximum of the PDF (finding mode)
                h_LI2(n) = xi(imax); % storing the mode
            end
        end
        
        clear adcp no_e i_max fmax f xi dt1 n imax mean_time mean_points Data % cleaning some variables
        %%      RAYLEIGH CRITERIA
        
        % defining rayleigh criteria parameters
        h_tres = 2.5;
        h_k_min = 5;
        
        % RC data folder is used to save the identified ridges and their time of occurence
        ...
        %       This is necessary in order to evaluate weekly statistics in a faster way. 
        ...
        for n = 1:1:size(t_reshape_R,2)
            ...
            if numel(H_R_reshape{n})>5 &&  not(isempty(h_SubSet))
                ...
                if not(isempty(max(locs(   (locs<3)&(pks>0.25)    ))))
                    DM(n) = max(locs(   (locs<3)&(pks>0.25)    ));      % finding the deepest mode estimate
                else
                    DM(n) = 0;
                    to_keep(n) = 0;
                end
