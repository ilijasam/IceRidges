...
% The script is modification of the original script "JP_year_long_histogram.m"
% Here we look only at one year and optimize the figures for usage in JP1

% - Weekly highest mode LI estimate is included
% - The script is optimized for 2012D data set, but it can be used for other seasons/locations

% UPDATE -------------- AN_LI_InterAnnual

% This update was made in order to make a plot of interannual LI developement that is to be used
% with "AN_WeeklyHistograms.m" to analyse data points one by one
...
%       calculate the mode from numerical pdf (this is seems to be the most precise way)
% for n = 1:length(t_LI)
%     [f,xi]=ksdensity(h_reshape(:,n)) ;
%     [fmax imax] = max(f);
%
% %     h_LI_pdf_mode(n) = xi(imax);
%     h_LI_mode(n) = xi(imax);
% end
